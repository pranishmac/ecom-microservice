...
## [2026-08-15 18:10] Restructure into feature-based packages

**Context:** With four domains now built (User, Product, Cart, Order), the codebase was flat in `com.app.ecom` for entities/controllers/services/repositories, with only `dto`/`mapper`/`exception` split out by technical layer — every domain's controller, service, entity, and repository sat in the same package as every other domain's.
**Decision:** Moved to feature-based packaging, confirmed via AskUserQuestion (preview of both options): `com.app.ecom.user`, `.product`, `.cart`, `.order`, each owning its own entity/controller/service/repository plus `dto`/`mapper` subpackages; exception types + `GlobalExceptionHandler` moved to `com.app.ecom.common.exception` (the one genuinely cross-cutting concern, used by every feature package). `EcomApplication` stays at the root `com.app.ecom` — Spring's component/entity scanning is automatic for all subpackages of wherever `@SpringBootApplication` lives, so no scanning configuration changed.
**Reason:** Everything about "orders" (or any other domain) now lives in one place instead of being spread across four parallel technical-layer packages — better cohesion as the app has grown past tutorial scale, and matches how most production Spring Boot codebases beyond a handful of entities are organized. `Address` was folded into `user` rather than given its own top-level package, since it has no independent controller/service and is only ever consumed through `User`.
**Alternatives considered:** Layer-based (`controller/`, `service/`, `repository/`, `entity/`, `enums/`, extending the existing `dto`/`mapper`/`exception` split consistently to everything else) — simpler mental model, rejected in favor of feature-based for the cohesion reasons above.
**Mechanics:** Physically moved via `git mv` (history-preserving), then every file's `package` declaration and cross-package imports were updated. This surfaced every genuine cross-domain dependency as an explicit import that previously didn't need one (same-package before the move): `Cart`/`Order` → `user.User`, `CartItem`/`OrderItem` → `product.Product`, `CartService`/`OrderService` → `user.UserRepository`, `OrderService` → `cart.CartRepository` and `product.ProductService`, `OrderMapper` → `user.Address`. None of this was a surprise — it's exactly the dependency graph already described in `notes.md`'s "when to call another service vs. its repository directly" — but having it as explicit imports rather than implicit same-package visibility makes the module boundaries honest instead of only conceptual. Verified with a full recompile (clean) and a live smoke test hitting one endpoint per module, including the cross-module chain (cart add → place order, which touches user, cart, product, and order together).
**Files touched:** all 49 source files (package declarations + imports); no logic changes.

## [2026-08-16 14:00] Add Spring Boot Actuator, isolated on a separate management port

**Context:** Needed production-grade operational endpoints — health, info, metrics, loggers, beans, shutdown — but this app has zero authentication anywhere, and several of those endpoints (`shutdown`, `beans`, `env`, `loggers` write access) are actively dangerous if reachable by anyone on the internet. Confirmed via AskUserQuestion: protect via a separate, non-public management port rather than adding Spring Security scoped to actuator.
**Decision:**
- Added `spring-boot-starter-actuator`, `micrometer-registry-prometheus`, and (test scope) `spring-boot-starter-actuator-test`.
- `management.server.port=8081` — actuator runs on an entirely separate embedded Tomcat connector from the public API (`server.port=8080`). Verified live: `/actuator/*` 404s on 8080, `/api/*` 404s on 8081 — two genuinely separate HTTP surfaces, not just a path prefix.
- `management.endpoints.web.exposure.include=health,info,metrics,prometheus,loggers,beans,env,mappings,shutdown` — explicit allowlist (Spring's own default is `health` only).
- `management.endpoint.shutdown.access=unrestricted` — required specifically for `shutdown`; every other exposed endpoint defaults to full read/write access the moment it's in the exposure list, but `shutdown` ships hard-disabled (`Access.NONE`) by the framework itself regardless of exposure, and needs this explicit per-endpoint override. Confirmed by inspecting `ShutdownEndpoint.class`'s `@Endpoint` annotation directly (`defaultAccess = Access.NONE`) rather than assumed, since Spring Boot 3.4 replaced the old boolean `management.endpoint.shutdown.enabled` flag with this `Access` enum model (`NONE`/`READ_ONLY`/`UNRESTRICTED`) — a real behavioral change between Spring Boot versions that would have silently produced a non-functional `shutdown` endpoint if guessed instead of verified.
- `management.endpoint.health.show-details=always` + `management.endpoint.health.probes.enabled=true` — full health detail and `/actuator/health/{liveness,readiness}` groups, safe specifically because health is only reachable on the isolated port.
- `spring-boot-maven-plugin`'s `build-info` goal added — populates `/actuator/info`'s `build` section (artifact, group, version, build time) from real build metadata instead of an empty response. Requires `mvn package` (binds to `prepare-package`), not just `compile` — verified `target/classes/META-INF/build-info.properties` is actually generated.
- `server.shutdown=graceful` + `spring.lifecycle.timeout-per-shutdown-phase=30s` — independent of the actuator `/shutdown` endpoint entirely; this is what an orchestrator's SIGTERM actually triggers, letting in-flight requests finish instead of dropping them.
**Reason:** The management port is "necessary but not sufficient" as a security boundary — Spring can bind actuator to 8081, but cannot itself guarantee 8081 is unreachable from outside the deployment network. That guarantee has to come from the deployer (firewall rule, container port mapping that only publishes 8080, k8s `NetworkPolicy`). This is the honest trade-off of the option chosen over adding Spring Security: zero risk to the 20+ existing unauthenticated endpoints, zero new attack surface on the app itself, but the actuator port's safety is an infrastructure responsibility this codebase cannot enforce alone.
**Alternatives considered:** Spring Security scoped to `/actuator/**` only (declined via AskUserQuestion) — stronger, defense-in-depth even if the management port were accidentally exposed, but a materially bigger change (first security code in the app, real risk of accidentally locking down the existing public API if the filter chain were scoped even slightly wrong) for a need better served by the deployment environment at this stage.
**Files touched:** pom.xml, application.properties
